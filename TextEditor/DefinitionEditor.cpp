...
void OpenF::performFullRelationWrite()
{
	stream->open(filepath, std::fstream::out | std::fstream::trunc);
...
void OpenF::printRelationToUser()
{
	for (auto i : keyRelations)
	{
		i->userdisplay();
...
void OpenF::saveRelations()
{
	std::cout << "Saving..." << std::endl;
	performFullRelationWrite();
	std::cout << "Save complete!" << std::endl;
}
...
{
	//When relating keys, we can't add desciption
	relator->clearDescription();
	bool cont = true;
